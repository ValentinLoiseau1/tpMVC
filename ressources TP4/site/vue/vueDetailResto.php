...
<h1><?= $unResto['nomR']; ?>
    ...
    <?php if ($aimer != false) { ?>
        <a href="./?action=aimer&idR=<?= $unResto['idR']; ?>" ><img class="aimer" src="asset/images/aime.png" alt="j'aime ce restaurant"></a>
    <?php } else { ?>
        <a href="./?action=aimer&idR=<?= $unResto['idR']; ?>" ><img class="aimer" src="asset/images/aimepas.png" alt="je n'aime pas encore ce restaurant"></a>
    <?php } ?>
...
<span id="note">
    <?php for ($i = 1; $i <= 5; $i++) { ?>
        <a class="aimer" href="./?action=noter&note=<?= $i ?>&idR=<?= $unResto['idR']; ?>" >
            <?php if ($i <= $noteMoy) { ?>
                <img class="note" src="asset/images/like.png" alt="">
            <?php } else {
                ?>
                <img class="note" src="asset/images/neutre.png" alt="line neutre">
            <?php } ?>
        </a>
    <?php } ?>
</span>
